...
public class DataPackRegistryForge {
    public static void register(CommonMod mod, final AddPackFindersEvent event){
        if (event.getPackType() == PackType.SERVER_DATA){
            mod.getDataPacks().ifPresent(dataPackLoader -> dataPackLoader.dataPacks.forEach(id -> register(id, event, mod)));
        }
    }

    private static void register(String id, final AddPackFindersEvent event, CommonMod mod) {
